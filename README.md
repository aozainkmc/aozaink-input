...
黄符是横向三格版面：左格、中格、右格。没有上一格、下一格、留空按钮、候选字列表和候选确认流程。
...
| 刻印符 | `slot1 = 刻`, `slot2 = 技能字`, `slot3 = 空` |
| 组合符 | 三格内包含 1-3 个技能字，不能包含数字或 `刻` |
| 废符 | 其他格式 |

当前技能字集合（12 玩法字）：`镇 封 退 引 火 雷 护 净 强 续 疾 广`，外加功能字 `刻` 与数字 `一..九`（暂兼容到 `1..9`）。专用数字模型需要 core 后续支持按 `sourceId` 加载多个在线轨迹模型实例。
