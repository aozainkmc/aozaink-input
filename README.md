...
- 调 `AozaiInkCoreApi.recognizer()` 取得识别结果
...
- 候选字选择流程（黄符第一版只显示 top1）

## 黄符三格 UI

黄符是横向三格版面：前两格为咒位，第三格为尾修槽。没有上一格、下一格、留空按钮、候选字列表和候选确认流程。
...
| 刻印符 | 三格无序，包含一个 `刻`；可配 1-2 个技能字，或 `续/强/广` 对已有刻印操作 |
| 组合符 | 三格内包含 1-3 个技能字，不能包含数字或 `刻` |
| 废符 | 其他格式 |

当前技能字集合（12 玩法字）：`镇 封 退 引 火 雷 护 净 斩 明 吸 魄`，外加功能字 `刻` 与数字 `一..九`（暂兼容到 `1..9`）。修饰字（4 个）：`强 续 广 穿`。专用数字模型需要 core 后续支持按 `sourceId` 加载多个在线轨迹模型实例。
